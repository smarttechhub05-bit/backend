...
async function loadWebsitePanel(role) {
  ...
  try {
    ...
    promotionForm?.querySelector('button[type="submit"]')?.insertAdjacentHTML('beforebegin', `<label>Promotion image<select name="image"><option value="">No image</option>${imageOptions}</select></label><p class="management-status" data-promotion-status></p>`);
    ...
    panel.querySelector('[data-promotion-form]').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; const formStatus = form.querySelector('[data-promotion-status]'); const values = Object.fromEntries(new FormData(form).entries()); values.active = form.active.checked; values.published = form.published.checked; const button = form.querySelector('button[type="submit"]'); button.disabled = true; try { const item = await requestJson('/api/website/admin/promotions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(values) }); form.parentElement.querySelector('.management-empty')?.remove(); form.insertAdjacentHTML('afterend', `<div class="management-item"><span><strong>${cmsEscape(item.title)}</strong><small>${item.active && item.published ? 'Published' : 'Draft / inactive'}</small></span><button type="button" data-promotion-delete="${item._id}">Unpublish</button></div>`); form.parentElement.querySelector(`[data-promotion-delete="${item._id}"]`).addEventListener('click', async (deleteEvent) => { try { await requestJson(`/api/website/admin/promotions/${item._id}`, { method: 'DELETE' }); deleteEvent.currentTarget.closest('.management-item').remove(); } catch (error) { formStatus.textContent = error.message; } }); form.reset(); formStatus.textContent = 'Promotion created.'; } catch (error) { formStatus.textContent = error.message; } finally { button.disabled = false; } });
...
async function loadGalleryPanel(role) {
  ...
  try {
    ...
    panel.querySelectorAll('[data-gallery-review]').forEach((button) => button.addEventListener('click', async () => {
      ...
      review.innerHTML = `<div class="gallery-review"><h3>${gallery.title} · Selection: ${gallery.selectionStatus}</h3><form class="management-form" data-gallery-upload><input name="title" placeholder="Media title"><input name="caption" placeholder="Caption"><select name="category"><option value="portfolio">Portfolio</option><option value="photography">Photography</option><option value="videography">Videography</option><option value="weddings">Weddings</option><option value="portraits">Portraits</option><option value="events">Events</option><option value="fashion">Fashion</option><option value="commercial">Commercial</option></select><input name="file" type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" required><button class="admin-button" type="submit">Upload media</button><p class="management-status"></p></form><p>Selected: ${gallery.mediaStats.selected} · Approved: ${gallery.mediaStats.approved} · Total: ${gallery.mediaStats.total}</p><select data-gallery-filter><option value="all">All</option><option value="selected">Selected</option><option value="not-selected">Not Selected</option><option value="approved">Approved</option></select><div class="management-list" data-gallery-items></div><h3>Revision Requests</h3><div class="management-list">${revisions.length ? revisions.map((revision) => `<div class="management-item"><span><strong>${revision.galleryItem?.title || 'Media item'}</strong><small>${revision.message} · ${revision.status}</small></span><select data-revision-id="${revision._id}"><option ${revision.status === 'open' ? 'selected' : ''}>open</option><option ${revision.status === 'in-progress' ? 'selected' : ''}>in-progress</option><option ${revision.status === 'completed' ? 'selected' : ''}>completed</option><option ${revision.status === 'cancelled' ? 'selected' : ''}>cancelled</option></select></div>`).join('') : '<p class="management-empty">No revision requests.</p>'}</div></div>`;
      ...
      renderItems();
      review.querySelector('[data-gallery-upload]').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; try { const item = await requestJson(`/api/galleries/${gallery._id}/media`, { method: 'POST', body: new FormData(form) }); gallery.items.push(item); form.reset(); form.querySelector('.management-status').textContent = 'Media uploaded successfully.'; renderItems(review.querySelector('[data-gallery-filter]').value); } catch (error) { form.querySelector('.management-status').textContent = error.message; } });
...
async function loadInquiryPanel(role) {
  if (!['superadmin', 'manager'].includes(role)) return;
  try {
    const inquiries = await requestJson('/api/contact');
    const render = (items) => items.length ? items.map((item) => `<div class="management-item"><span><strong>${cmsEscape(item.name)}</strong><small>${cmsEscape(item.email)} · ${formatStudioDate(item.createdAt)} · ${item.status}</small><br>${cmsEscape(item.message)}</span><select data-inquiry-status="${item._id}"><option value="new" ${item.status === 'new' ? 'selected' : ''}>New</option><option value="read" ${item.status === 'read' ? 'selected' : ''}>Read</option><option value="archived" ${item.status === 'archived' ? 'selected' : ''}>Archived</option></select></div>`).join('') : '<p class="management-empty">No contact inquiries yet.</p>';
    const panel = addDashboardPanel('Contact Inquiries', `<div class="management-list" data-inquiry-list>${render(inquiries)}</div>`);
    panel.id = 'contact-inquiries';
    panel.querySelectorAll('[data-inquiry-status]').forEach((select) => select.addEventListener('change', async () => { try { await requestJson(`/api/contact/${select.dataset.inquiryStatus}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: select.value }) }); } catch (error) { select.closest('.management-item').insertAdjacentHTML('beforeend', `<p class="management-status">${cmsEscape(error.message)}</p>`); } }));
  } catch (error) {
    const panel = addDashboardPanel('Contact Inquiries', `<p class="management-status">Unable to load inquiries: ${cmsEscape(error.message)}</p>`);
    panel.id = 'contact-inquiries';
  }
}
