...
async function loadWebsitePanel(role) {
  ...
  try {
    ...
    renderMedia(media);
    mediaPanel.querySelector('[data-media-form]').addEventListener('submit', async (event) => { event.preventDefault(); const form = event.currentTarget; const file = form.querySelector('[name="file"]').files[0]; const allowedTypes = ['image/jpeg', 'image/png', 'image/webp', 'video/mp4', 'video/webm', 'video/quicktime']; if (!file || !allowedTypes.includes(file.type)) { status.textContent = 'Unsupported file. Convert camera RAW files such as CR2 to JPG, PNG, or WebP first.'; return; } const formData = new FormData(form); try { const asset = await requestJson('/api/website/admin/media', { method: 'POST', body: formData }); media.push(asset); renderMedia(media); form.reset(); status.textContent = 'Media uploaded successfully.'; } catch (error) { status.textContent = error.message; } });
